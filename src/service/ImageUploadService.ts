...
export default class ImageUploadService {
		...
		try {
			...
			if (imageBuffer) {
				...
				if (this.premiumNIP98User) {
					let base64encodedEventString = await nip98.getToken(
						ImageUploadService.UPLOAD_ENDPOINT,
						"post",
						(authEvent) =>
							finalizeEvent(
								authEvent,
	...
	isFileSizeOverLimit(file: ArrayBuffer): boolean {
		...
		if (file.byteLength > maxSizeInBytes) {
			...
			new Notice(
				"❌ Inline image size exceeds the limit. Will not upload.",
			);
			return true;
		}
		return false;
