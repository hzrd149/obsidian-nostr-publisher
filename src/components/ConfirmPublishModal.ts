...
export default class ConfirmPublishModal extends Modal {
  ...
  async onOpen() {
    ...
    let selectedAccount = this.plugin.accounts.active;
    ...
    new Setting(contentEl)
      ...
      .addDropdown((dropdown) => {
        for (const account of this.plugin.accounts.accounts) {
          dropdown.addOption(account.id, account.metadata?.name || account.id);
        }
        if (this.plugin.accounts.active)
          dropdown.setValue(this.plugin.accounts.active.id);

        dropdown.onChange(async (value) => {
          selectedAccount = this.plugin.accounts.getAccount(value);
          new Notice(`${selectedAccount?.metadata?.name} selected`);
        });
      });
    ...
    let publishButton = new ButtonComponent(contentEl)
      ...
      .onClick(async () => {
        if (
          confirm(
            `Are you sure you want to publish this note ${publishAsDraft ? "as a draft" : "publicly"} to Nostr?`,
          )
        ) {
          ...
          setTimeout(async () => {
            try {
              ...
              // @ts-expect-error
              let res = await this.pool.publishNote(
                fileContent,
                this.file,
                summary,
